...
if(new URLSearchParams(location.search).get('original')==='1'){
  ...
  note.textContent='Original GoldenEye characters selected. ';
  const link=document.createElement('a');link.textContent='Switch to the Gameslop cast';
  const url=new URL(location.href);url.searchParams.delete('original');link.href=url.href;
  link.style.textDecoration='underline';note.append(link);
}
$('play').addEventListener('click',()=>{
  ...
  if(query.get('original')==='1')options.set('original','1');
