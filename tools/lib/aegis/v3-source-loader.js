...
const V3_JSON_OPTIONS = Object.freeze({
  maxDepth: 32,
  maxObjectFields: 64,
  rejectNegativeZero: true,
});
const STABLE_ID = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,63}$/;
const MISSION_ID = /^[a-z][a-z0-9-]{0,63}$/;
...
function validateSourcePath(value, diagnosticPath) {
  ...
  if (!value.endsWith(".json")) {
    fail("SOURCE_REFERENCE", diagnosticPath, "V3 JSON source references must use a lowercase .json extension");
  }
  return value;
}
...
function validateReference(value, diagnosticPath, extraKey) {
  ...
  if (extraKey === "schemaVersion") {
    if (value.schemaVersion !== 1 && value.schemaVersion !== 2) {
      fail("SCHEMA_LITERAL", pointerJoin(diagnosticPath, "schemaVersion"), "Map schemaVersion must be 1 or 2");
    }
  }
...
function validateV3SourceManifest(value) {
  ...
  if (value.schemaVersion !== 3) {
    fail("SCHEMA_LITERAL", "/schemaVersion", "V3 campaign manifest schemaVersion must be exactly 3");
  }
  ...
  if (value.sourceKind !== "campaign") {
    fail("SCHEMA_LITERAL", "/sourceKind", "V3 sourceKind must be exactly campaign");
  }
  if (!APPROVAL_STATES.includes(value.approvalState)) {
    fail("SCHEMA_LITERAL", "/approvalState", "Unknown v3 approval state");
  }
  ...
  if (!Array.isArray(value.missions) || value.missions.length < 1 || value.missions.length > MAX_MISSIONS) {
    fail("SCHEMA_ARRAY", "/missions", "V3 missions must contain 1-" + MAX_MISSIONS + " records");
  }
  ...
  value.missions.forEach(function (mission, index) {
    ...
    if (typeof mission.id !== "string" || !MISSION_ID.test(mission.id)) {
      fail("SCHEMA_STRING", pointerJoin(base, "id"), "Mission IDs must be lowercase portable IDs");
    }
  ...
  function register(reference, diagnosticPath) {
    if (sourceOwners.has(reference.source)) {
      fail("SCHEMA_DUPLICATE_SOURCE", pointerJoin(diagnosticPath, "source"), "V3 references must use unique source paths");
    }
    sourceOwners.set(reference.source, diagnosticPath);
  }
  STANDARD_REFERENCE_KEYS.forEach(function (key) {
    register(value[key], pointerJoin("/", key));
  });
...
function parseV3JsonBytes(bytes, label) {
  const buffer = Buffer.isBuffer(bytes) ? bytes : Buffer.from(bytes);
  if (buffer.length > MAX_SOURCE_BYTES) {
    fail("SOURCE_SIZE", "/", (label || "V3 source") + " exceeds the one-MiB source limit");
  }
  return parseStrictJsonBytes(buffer, label || "V3 source", V3_JSON_OPTIONS);
}
...
function resolveDeclaredFile(root, relativePath, diagnosticPath, registry) {
...
function readRegularFile(file, diagnosticPath, maximumBytes, registry) {
  ...
  try {
    ...
    registerPhysicalIdentity(registry, {
      real: file.realIdentity,
      inode: openedIdentity,
    }, diagnosticPath);
...
function preflightV3SourceTree(options) {
  if (!isPlainRecord(options)) fail("SOURCE_READ", "/", "V3 preflight options must be a plain object");
  ...
  Object.getOwnPropertyNames(options).forEach(function (key) {
    if (!allowedOptions.has(key)) throw new TypeError("Unknown v3 preflight option " + key);
  });
  const sourceRoot = resolveDirectory(options.sourceRoot, "/", "V3 source root");
  ...
  const manifestSource = options.manifestSource === undefined ? "schema-version.json" : options.manifestSource;
  validateSourcePath(manifestSource, "/manifestSource");
  const manifestFile = resolveDeclaredFile(sourceRoot, manifestSource, "/manifestSource", registry);
  const manifestBytes = readRegularFile(manifestFile, "/manifestSource", MAX_SOURCE_BYTES, registry);
  const manifest = validateV3SourceManifest(parseV3JsonBytes(manifestBytes, manifestSource));

  const provenance = [];
  const resolvedPaths = [];
  let abiDescriptorBytesBase64 = null;
  function loadJsonReference(reference, diagnosticPath, kind, id) {
    const file = resolveDeclaredFile(sourceRoot, reference.source, pointerJoin(diagnosticPath, "source"), registry);
    ...
    const parsed = parseV3JsonBytes(bytes, reference.source);
    if (kind === "abiDescriptor") abiDescriptorBytesBase64 = bytes.toString("base64");
    provenance.push({ kind: kind, id: id, source: reference.source, sha256: reference.sha256 });
  ...
  const presentationCatalog = loadJsonReference(
    manifest.presentationCatalog,
    "/presentationCatalog",
    "presentationCatalog",
    "presentationCatalog"
  );
  if (manifest.approvalState === "production-approved" && presentationCatalog.schemaVersion === 1) {
    fail(
      "PRESENTATION_APPROVAL",
      "/presentationCatalog",
      "Presentation catalog schema v1 is a procedural Candidate-BAL placeholder and cannot be production approved"
    );
  }
  ...
  const missions = manifest.missions.map(function (mission, index) {
    ...
    if (mission.mapProofSupplement) {
      record.mapProofSupplement = loadJsonReference(
        mission.mapProofSupplement,
        pointerJoin(base, "mapProofSupplement"),
        "mapProofSupplement",
        mission.id
      );
    }
  ...
  function verifyRepositoryRecord(record, diagnosticPath, kind) {
    ...
    const file = resolveDeclaredFile(repositoryRoot, record.repositoryPath, pathField, registry);
  ...
  let lockCoverage;
  if (options.normalizedLockTree !== undefined) {
    lockCoverage = validateLockCoverage(annex, options.normalizedLockTree, {
      semanticArrayPaths: options.semanticArrayPaths,
    });
  } else if (options.semanticArrayPaths !== undefined) {
    throw new TypeError("semanticArrayPaths requires normalizedLockTree");
  }
  ...
  const result = {
    ...
    manifest: manifest,
    annex: annex,
    lockCoverageStatus: lockCoverage === undefined ? "deferred-until-normalization" : "validated",
    normalizedSource: {
    ...
    verifiedRawSources: {
      abiDescriptorBase64: abiDescriptorBytesBase64,
    },
    paths: {
      manifest: manifestFile.path,
      declared: resolvedPaths,
    },
  };
  if (lockCoverage !== undefined) result.lockCoverage = lockCoverage;
  return deepFreeze(result);
}

module.exports = Object.freeze({
  MAX_SOURCE_BYTES: MAX_SOURCE_BYTES,
  V3_JSON_OPTIONS: V3_JSON_OPTIONS,
  parseV3JsonBytes: parseV3JsonBytes,
  preflightV3SourceTree: preflightV3SourceTree,
  sha256Reference: sha256Reference,
  validateSourcePath: validateSourcePath,
  validateV3SourceManifest: validateV3SourceManifest,
});
