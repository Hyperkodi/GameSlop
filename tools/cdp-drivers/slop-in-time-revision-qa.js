...
module.exports=async(cdp,evaluate,sleep)=>{
 ...
 for(let era=0;era<6;era++){
   ...
   await evaluate(`(()=>{const e=__gameslop.engine,s=e.state,p=s.players[0];s.status='playing';s.gate=0;s.camera=0;s.cameraY=0;p.x=170;p.y=416;e.tick();const base=s.enemies[0];s.enemies=SlopInTime.content.enemyKinds.map((kind,i)=>({...base,id:100+i,kind,x:270+i*135,y:430,z:kind==='flyer'?85:kind==='thrower'?60:0,hp:35,maxHp:35,defend:kind==='guard'?1:0}));s.props=[];s.pickups=[];s.traps=[];s.status='paused';__gameslop.renderer.draw(s,1);})()`);await shot('enemies-'+(era+1));
 }
