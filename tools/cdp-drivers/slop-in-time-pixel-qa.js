...
module.exports=async(cdp,evaluate,sleep)=>{
  ...
  for(let era=0;era<6;era++){
    const result=await evaluate(`(()=>{
      ...
      s.camera=art.width-960;s.cameraY=600;r.draw(s,0);const end=ctx.getImageData(850,180,100,100).data;let occupied=0;for(let i=0;i<end.length;i+=4)if(end[i]+end[i+1]+end[i+2]>45)occupied++;
