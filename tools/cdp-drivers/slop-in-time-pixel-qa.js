...
module.exports=async(cdp,evaluate,sleep)=>{
  ...
  const info=await evaluate('__gameslop.renderer.inspect()');assert.deepEqual(info.world,[480,270]);assert.equal(info.panoramas.length,6);assert.ok(info.panoramas.every(p=>p[0]===4352&&p[1]===340));assert.ok(info.redPalette);
  ...
  for(let era=0;era<6;era++){
    const result=await evaluate(`(()=>{
      ...
      for(const offset of [1408,2816]){s.camera=offset/ratio;r.draw(s,0);const a=ctx.getImageData(0,170,960,140).data;s.camera+=2;r.draw(s,0);const b=ctx.getImageData(0,170,960,140).data;let difference=0;for(let i=0;i<a.length;i+=4)difference+=Math.abs(a[i]-b[i])+Math.abs(a[i+1]-b[i+1])+Math.abs(a[i+2]-b[i+2]);differences.push(difference/(a.length*.75));}
      s.camera=art.width-960;r.draw(s,0);const end=ctx.getImageData(850,180,100,100).data;let occupied=0;for(let i=0;i<end.length;i+=4)if(end[i]+end[i+1]+end[i+2]>45)occupied++;
    ...
    await evaluate(`(()=>{const e=__gameslop.engine;e.start();for(let i=0;i<${era};i++){e.state.status='clear';e.advance();}e.state.gate=9;const p=e.state.players[0];p.x=SlopInTime.stages[e.state.stage].encounters[9]-300;p.invincible=100;e.tick();const b=e.state.enemies.find(x=>x.boss);e.state.enemies=[b];e.state.props=[];b.hp=b.maxHp*.28;b.superTimer=0;b.cooldown=0;for(let i=0;i<70;i++)e.tick();e.state.transition=0;window.__boss=b;e.state.status='paused';__gameslop.renderer.draw(e.state,0);})()`);
    assert.ok(await evaluate('__boss.enraged&&__gameslop.engine.state.hazards.length>=2'),'red boss and superpower era '+era);
    // Freeze the same pose and compare actual sprite pixels before/after rage.
    const redPixels=await evaluate(`(()=>{const s=__gameslop.engine.state,r=__gameslop.renderer,b=__boss,ctx=document.querySelector('canvas').getContext('2d'),x=Math.max(0,Math.round(b.x-s.camera-75)),y=Math.round(b.y-165);b.hp=b.maxHp;r.draw(s,0);const a=ctx.getImageData(x,y,150,160).data;b.hp=b.maxHp*.28;r.draw(s,0);const z=ctx.getImageData(x,y,150,160).data;let changed=0;for(let i=0;i<a.length;i+=4)if(z[i]>z[i+1]*2&&z[i]>z[i+2]*2&&Math.abs(z[i]-a[i])+Math.abs(z[i+1]-a[i+1])>25)changed++;return changed;})()`);
    ...
    await evaluate("document.querySelector('#overlay').hidden=true");await shot('era-'+(era+1)+'-boss');console.log('PASS long panorama, seam continuity, red boss and unique power: era '+(era+1));
  }
