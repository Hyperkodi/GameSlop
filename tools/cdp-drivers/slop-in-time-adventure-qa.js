...
module.exports=async(cdp,evaluate,sleep)=>{
  ...
  for(let era=0;era<6;era++){
    ...
    await evaluate(`(()=>{const e=__gameslop.engine;e.start();for(let i=0;i<${era};i++){e.state.status='clear';e.advance();}e.state.gate=1;const p=e.state.players[0];p.x=SlopInTime.stages[${era}].encounters[1]-300;p.invincible=100;e.tick();Object.assign(e.state.enemies[0],{kind:'flyer',z:82});e.state.transition=0;e.state.story=null;e.state.status='paused';})()`);
