...
import {WEAPONS,CHAPTERS,DIFFICULTIES,BOONS,UPGRADE_RARITIES,LEGENDARY,CHEST_CAP,CHEST_INTERVAL,defaultSave,normalizeSave,syncChests,openChests,upgradeCost,upgradeWeapon,weaponUnlocked,unlockedChapter,unlockedDifficulty,weapon,clamp,chestTotal,rankUp,upgradeFoundry,refine,unlockWeapon,highestUnlocked,SPECIALS,buySpecial,equipSpecial} from './data.mjs';
...
function modal(html,id){releaseInputs();sound.pause();if($('#modal').hidden)lastFocus=document.activeElement;dialog=id;$('#modal').dataset.kind=id;$('#modal-content').innerHTML=html;$('#modal').hidden=false;updateFullscreenButtons();$('.modal').focus();requestFrame();}
...
function renderHome(){
 ...
 if(tab==='expedition'){
  html+='<div class="display-bar"><span>See the whole arena.</span><button class="fullscreen-button" data-action="fullscreen" aria-label="Enter fullscreen" aria-pressed="false">\u26f6 Full screen</button></div>';
 ...
 if(tab==='foundry')html+=intro('THE FOUNDRY','A stronger foundation.','Four permanent tracks improve every future run, and specials are bought here.')+foundryView(save)+specialsView(save,icon);
 ...
 ['coin','Aim. Fire. Cool down.','Hold the <strong>thumb joystick</strong> off center to aim and fire. Release to cool the cannon. On desktop, hold the mouse button in the arena; arrows / WASD aim and fire too. <strong>R</strong> vents heat; <strong>E or Space</strong> triggers your special. Glowing rings mark vulnerable sections: hit them for 65% extra damage.'],
 ...
 ['chain','Specials','Specials are bought in the Foundry with coins: Market Crash after clearing level 3, then Liquidation Wave, The Halving and Hard Fork Bomb. Equip one per run. Dealing damage charges it; when it reads READY, tap the star button or press <strong>E or Space</strong>. Ranks 2 and 3 hit harder and charge faster.'],
 ...
 $('#page').innerHTML=html;updateVault();updateFullscreenButtons();
}
...
function showChoices(){modal(`<div class="upgrade-heading"><span class="eyebrow">BATTLE FROZEN · ${run.weapons.length} / ${run.slots||6} WEAPONS</span><h2 id="modal-title">${run.openingChoice?'Choose your first weapon':'Choose an upgrade'}</h2><p>${run.openingChoice?'Hold the aim stick or mouse button to fire. Release to cool; R vents. Choose an upgrade to begin.':`Green 60% · Blue 28% · Red 10% · Gold 2% per option`}</p></div><div class="choice-grid">${run.choices.map(c=>`<button class="choice ${c.rarity}" data-choice="${c.id}">${icon(c.weapon)}<span class="eyebrow">${UPGRADE_RARITIES.find(t=>t.id===c.rarity)?.name||'UPGRADE'}</span><b>${escapeText(c.title)}</b><p>${escapeText(c.description)}</p><small>${c.kind==='unlock'?'COLLECT WEAPON':weapon(c.weapon).name.toUpperCase()} →</small></button>`).join('')}</div><div class="modal-actions"><button class="secondary" data-action="reroll" ${run.rerolls?'':'disabled'}>↻ Reroll · ${run.rerolls} left</button><button class="secondary" data-action="save-exit">Return to menu</button></div>`,'choice');}

function pause(){releaseInputs();if(!run||run.state!=='playing')return;run.state='paused';sound.pause();persist();modal(`<span class="eyebrow">${run.mode==='tournament'?'ENDLESS TOURNAMENT':'LEVEL '+String(run.chapter+1).padStart(2,'0')+' / '+run.difficulty.toUpperCase()} &middot; WAVE ${run.wave}${run.mode==='tournament'?'':' / '+CHAPTERS[run.chapter].waves}</span><h2 id="modal-title">${escapeText(routeStyleFor(run.chapter,run.wave,run.mode).name)}</h2><p>Your snake, projectiles and cooldowns are paused. Your run is saved.</p><div class="modal-actions"><button class="secondary" data-action="save-exit">Return to menu</button><button class="secondary" data-action="audio-settings">Audio & settings</button><button class="primary" data-action="continue">Continue siege →</button></div>`,'pause');}
function showRevive(){
...
document.addEventListener('click',async e=>{
 ...
 if(b.dataset.buySpecial){if(buySpecial(save,b.dataset.buySpecial)){sound.event('upgrade');persist();renderHome();}return;}
...
function updateFullscreenButtons(){
 ...
 for(const button of document.querySelectorAll('[data-action="fullscreen"]')){
  ...
  if(button.classList.contains('battle-fullscreen'))button.querySelector('small').textContent=active?'Exit screen':'Full screen';
  else button.textContent=active?'\u26f6 Exit fullscreen':'\u26f6 Full screen';
 }
}
async function toggleFullscreen(){
 releaseInputs();
 try{
  if(document.fullscreenElement||document.webkitFullscreenElement){const exit=document.exitFullscreen||document.webkitExitFullscreen;if(exit)await exit.call(document);}
  else {const root=document.documentElement,enter=root.requestFullscreen||root.webkitRequestFullscreen;if(!enter)throw new Error('Fullscreen unavailable');await enter.call(root);}
  updateFullscreenButtons();
 }catch{toast('Use Add to Home Screen for fullscreen on this browser.');}
}
...
let stickPointer=null,canvasPointer=null,keys=new Set(),stickX=0,stickY=0,gamepadAiming=false;
const stick=$('#joystick'),thumb=$('#stick-thumb');
function releaseInputs(){if(typeof keys!=='undefined')keys.clear();stickX=stickY=0;if(typeof thumb!=='undefined')thumb.style.transform='translate(0px,0px)';if(run)run.manual=false;if(typeof stick!=='undefined'&&stickPointer!==null){const id=stickPointer;stickPointer=null;try{stick.releasePointerCapture(id);}catch{}}if(canvasPointer!==null){const id=canvasPointer;canvasPointer=null;try{$('#arena').releasePointerCapture(id);}catch{}}$('#aim-mode').textContent='AIM TO FIRE';}
function moveStick(e){if(e.pointerId!==stickPointer||!run||run.state!=='playing')return;const b=stick.getBoundingClientRect(),dx=e.clientX-b.left-b.width/2,dy=e.clientY-b.top-b.height/2,len=Math.hypot(dx,dy),max=b.width*.33,f=Math.min(1,len/max);stickX=len?dx/len*f:0;stickY=len?dy/len*f:0;thumb.style.transform=`translate(${stickX*max}px,${stickY*max}px)`;if(f<.16){run.manual=false;$('#aim-mode').textContent='AIM TO FIRE';}else{run.manual=true;run.aimX=clamp(240+stickX*205,45,435);run.aimY=clamp(330+stickY*235,90,550);$('#aim-mode').textContent='FOCUS FIRE';}}
...
function activateSpecial(){if(run&&ultimate(run)){sound.event('ultimate');updateHUD();}else if(run?.state==='playing')toast(run.special||run.mode==='tournament'?'Deal damage to charge your special attack.':'Equip a special in the Foundry first.');}
$('#ultimate').addEventListener('pointerdown',e=>{e.preventDefault();activateSpecial();});
$('#vent').addEventListener('pointerdown',e=>{e.preventDefault();if(run&&vent(run))updateHUD(true);});
$('#vent').addEventListener('click',e=>{if(e.detail===0&&run)vent(run);});
$('#ultimate').addEventListener('click',e=>{if(e.detail===0)activateSpecial();});
...
function aimCanvas(e){if(run?.state!=='playing')return;if(e.pointerId!==canvasPointer)return;const rect=$('#arena').getBoundingClientRect();run.aimX=clamp((e.clientX-rect.left)/rect.width*480,35,445);run.aimY=clamp((e.clientY-rect.top)/rect.height*760,80,550);run.manual=true;$('#aim-mode').textContent='FOCUS FIRE';}
$('#arena').addEventListener('pointermove',aimCanvas);// Releasing any aiming pointer stops fire. Hover never pulls the trigger.
for(const type of ['pointerup','pointercancel','lostpointercapture','pointerleave'])$('#arena').addEventListener(type,e=>{if(canvasPointer!==null&&e.pointerId===canvasPointer)releaseInputs();});
document.addEventListener('keydown',e=>{
 ...
 if(e.altKey||e.ctrlKey||e.metaKey||e.isComposing||e.target.closest('input,textarea,select,[contenteditable="true"]'))return;
 if(e.key.toLowerCase()==='e'||e.key===' '){e.preventDefault();if(!e.repeat)activateSpecial();return;}
 if(e.key.toLowerCase()==='r'){e.preventDefault();vent(run);return;}
 if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','w','a','s','d'].includes(e.key)){e.preventDefault();keys.add(e.key);}
});document.addEventListener('keyup',e=>{if(!keys.delete(e.key))return;if(!keys.size&&stickPointer===null&&canvasPointer===null&&!gamepadAiming)releaseInputs();});
...
function frame(now){
 ...
 if(run&&!$('#battle').hidden){
  if(run.state==='playing'){
   ...
   const gp=navigator.getGamepads?.()[0],padActive=gp&&Math.hypot(gp.axes[0]||0,gp.axes[1]||0)>.18;if(padActive){kx=gp.axes[0];ky=gp.axes[1];gamepadAiming=true;}else if(gamepadAiming){gamepadAiming=false;if(!keys.size&&stickPointer===null&&canvasPointer===null)run.manual=false;}if(gp?.buttons[0]?.pressed)ultimate(run);if(gp?.buttons[1]?.pressed)vent(run);
   ...
   const events=[];for(let step=0;step<(save.settings.speed===2?2:1)&&run.state==='playing';step++){tick(run,dt);events.push(...run.events);}for(const e of events){sound.event(e.type,e.weapon);if(e.type==='ultimate'){const shell=$('.arena-shell');shell.classList.remove('shake');void shell.offsetWidth;shell.classList.add('shake');}if(e.type==='wave'){clearTimeout(calloutTimer);$('#battle-callout').textContent=`WAVE ${e.wave} · ${bossForChapter(run.chapter).name}`;$('#battle-callout').style.opacity='1';calloutTimer=setTimeout(()=>$('#battle-callout').style.opacity='0',2000);}}
   if(run.state==='choice'){showChoices();persist();}else if(run.state==='revive'){showRevive();}else if(['won','lost'].includes(run.state))finish();
  }
  $('#aim-mode').textContent=run.overheated?'RELEASE TO COOL':run.manual?'FOCUS FIRE':'AIM TO FIRE';$('#vent').disabled=run.state!=='playing'||run.heat<.15||run.ventUntil>run.time;renderer.draw(run,run.time,save.settings.reduced);sound.update(run.state==='playing');uiTime+=dt;if(uiTime>.2){updateHUD();uiTime=0;}saveTime+=dt;if(saveTime>3){persist();saveTime=0;}
 }
...
window.slopSurvivor={snapshot:()=>JSON.parse(JSON.stringify({run,save,storageOK,dialog})),version:'3.2.7'};
