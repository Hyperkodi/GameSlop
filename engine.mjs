...
export function createRun(save,chapter=0,difficulty='easy',seed=Date.now()){
 ...
 return {version:VERSION,rules:2,combatVersion:3,heat:0,overheated:false,ventUntil:0,mode:'campaign',chapter,difficulty,seed:(seed>>>0)||1,time:0,state:'boon',wave:0,segments:[],bullets:[],effects:[],numbers:[],weapons:[],deck:WEAPONS.filter(w=>weaponUnlocked(save,w)).map(w=>w.id),baseLevels:{...save.levels},baseRanks:{...save.ranks},foundry:{...save.foundry},slots:arsenalSlots(save),special:save.specials?.equipped??null,specialRank:save.specials?.owned?.[save.specials?.equipped]||1,boons:[],boonOptions:boonOptionsFor(difficulty,(seed>>>0)||1),boonsLeft:DIFFICULTIES.find(d=>d.id===difficulty).boons,choices:[],pending:0,headDistance:180,health,maxHealth:health,score:0,kills:0,waveKills:0,nextChest:3,castId:0,charge:0,slowUntil:0,slowAmount:0,aimX:240,aimY:130,manual:false,heroX:240,heroY:640,events:[],resultApplied:false,rerolls:d.rerolls,lastChoice:0};
}
...
export function reviveTournament(r){
 ...
 if(isActionRun(r))for(const snake of r.snakes||[])snake.distance=-snake.id*120;
...
export function spawnWave(r){
 ...
 if(isActionRun(r))spawnActionWave(r,c,d);
...
export function hit(r,s,w,amount=w.damage){if(!s||s.hp<=0)return;const hpBefore=s.hp,crit=w.guaranteedCrit||random(r)<w.crit;let dmg=amount*(isActionRun(r)&&weakSection(r,s)?1.65:1)*(crit?w.mult:1)*(s.markedUntil>r.time?1.25:1)*(1+.08*(s.slipStacks||0));if(s.armor&&!crit)dmg*=.68;if(!s.head&&!crit&&encounterPhase(r).id==='shutters')dmg*=.7;if(r.mode==='tournament')dmg=Math.min(s.hp,dmg);if(w.id==='sniper'&&w.legendary&&s.hp<s.maxHp*.15)dmg=Math.max(dmg,s.hp);s.hp-=dmg;if(s.markedUntil>r.time&&s.hp>0&&s.hp<=s.maxHp*.1&&r.weapons.some(x=>x.id==='oracle'&&x.legendary)){dmg+=s.hp;s.hp=0;}s.flash=.1;w.totalDamage+=dmg;r.charge=Math.min(100,r.charge+Math.min(dmg,hpBefore)*.012*chargeRate(r)/(r.mode==='tournament'?1:encounterHealth(r.chapter+1)));r.score+=Math.round(dmg*.12);if(r.numbers.length<55)r.numbers.push({x:s.x+(random(r)-.5)*15,y:s.y,text:Math.round(dmg),crit,color:weapon(w.id).color,life:.65});}
...
export function fire(r,w,t){
 ...
 const point=isActionRun(r)?{x:r.aimX,y:r.aimY}:aimPoint(t,r.manual?r.aimX:r.heroX,r.manual?r.aimY:r.heroY);t={...t,x:point.x,y:point.y};
 const muzzle=isActionRun(r)?firingPoint(r):{x:r.heroX,y:r.heroY-25};
 const base=weapon(w.id),color=base.color;r.events.push({type:'fire',weapon:w.id});const angle=Math.atan2(t.y-muzzle.y,t.x-muzzle.x);
 if(isActionRun(r)&&['bomb','burn'].includes(base.type)){fx(r,{type:'airstrike',weapon:w.id,x:t.x,y:t.y,r:w.radius,color,life:.7,napalm:base.type==='burn',trigger:false});return;}
 if(isActionRun(r)&&['chain','oracle','flashloan','printer','swarm','dragon'].includes(base.type)&&sectionDistance(r.segments.find(s=>s.id===t.id),t.x,t.y)>85)return;
 ...
 switch(base.type){
  ...
  case 'printer': {
   ...
   homing(r,w,isActionRun(r)?70:r.heroX,isActionRun(r)?580:r.heroY-25,t,'printer');break;
  }
  case 'sniper': {
   const targets=r.segments.filter(s=>visible(s)&&(!isActionRun(r)||sectionDistance(s,t.x,t.y)<38)).sort((a,b)=>a.hp-b.hp||a.id-b.id).slice(0,1+w.pierce);for(const s of targets){hit(r,s,w);fx(r,{type:'beam',weapon:w.id,x:isActionRun(r)?muzzle.x:r.heroX,y:isActionRun(r)?muzzle.y:r.heroY-18,x2:s.x,y2:s.y,color,life:.2});}break;
  }
  case 'halving': {
  ...
  case 'slippage': {
   const targets=r.segments.filter(visible).sort((a,b)=>sectionDistance(a,t.x,t.y)-sectionDistance(b,t.x,t.y)).slice(0,Math.max(1,w.chains-3));for(const s of targets){s.slipStacks=Math.min(w.legendary?10:5,(s.slipStacks||0)+1);s.slipUntil=r.time+5;hit(r,s,w);}fx(r,{type:'field',weapon:w.id,x:t.x,y:t.y,r:w.radius,color,life:.4});break;
  }
  case 'copium':hazard(r,w,t.x,t.y,w.radius,4,w.damage*.4,'copium');break;
  case 'trap': {
   const head=r.segments.find(visible),distance=(head?.distance??r.headDistance)+36;const place=(distance)=>{const p=isActionRun(r)?{x:t.x,y:t.y}:pathPoint(distance,r.chapter);fx(r,{type:'trap',id:++r.castId,weapon:w.id,distance,x:p.x,y:p.y,r:w.radius,color,life:8,armed:true});};place(distance);break;
  }
  case 'nuke': {
  ...
  case 'lambo': {
   const head=(isActionRun(r)?r.segments.find(s=>visible(s)&&s.snakeId===t.snakeId):null)||r.segments.find(visible),start=head.distance??r.headDistance;for(let i=0;i<w.count;i++){const snakeId=head.snakeId||0,p=isActionRun(r)?routePoint(r,start,snakeId):pathPoint(start,r.chapter);fx(r,{type:'lambo',snakeId,id:++r.castId,weapon:w.id,distance:start,start,travel:0,range:Math.min(2600,360+w.level*32),direction:-1,returning:false,hits:[],x:p.x,y:p.y,angle:p.angle,color,life:14});}break;
  }
  case 'flashloan': {
 ...
 if(base.type==='beam'){
  const origin=isActionRun(r)?muzzle:{x:r.heroX,y:r.heroY},dx=t.x-origin.x,dy=t.y-origin.y,len=Math.max(1,Math.hypot(dx,dy)),ux=dx/len,uy=dy/len;
  const list=r.segments.filter(s=>visible(s)&&sectionPoints(s).some(p=>onBoard(p)&&((p.x-origin.x)*ux+(p.y-origin.y)*uy)>0&&Math.abs((p.x-origin.x)*uy-(p.y-origin.y)*ux)<19+Math.min(8,w.tier))).sort((a,b)=>Math.hypot(a.x-r.heroX,a.y-r.heroY)-Math.hypot(b.x-r.heroX,b.y-r.heroY));
  list.slice(0,w.legendary?200:w.pierce+1).forEach(s=>hit(r,s,w));fx(r,{type:'beam',weapon:w.id,x:isActionRun(r)?muzzle.x:r.heroX,y:isActionRun(r)?muzzle.y:r.heroY-18,x2:origin.x+ux*780,y2:origin.y+uy*780,color,tier:w.tier,life:.23});
 }
 if(base.type==='chain'){
  let cur=r.segments.find(s=>s.id===t.id),from=isActionRun(r)?{x:t.x,y:75}:{x:r.heroX,y:r.heroY};const seen=[];for(let i=0;i<w.chains*(w.legendary?2:1);i++){if(!cur)break;hit(r,cur,w);seen.push(cur.id);fx(r,{type:'chain',x:from.x,y:from.y,x2:cur.x,y2:cur.y,color,life:.35});from=cur;
 ...
 if(base.type==='field'){area(r,w,t.x,t.y,w.radius);fx(r,{type:'field',weapon:w.id,x:t.x,y:t.y,r:w.radius,color,life:2.7,slow:w.slow});if(w.legendary)hazard(r,w,t.x,t.y,w.radius,3,w.damage*.4,'rug');}
 ...
 if(base.type==='swarm'){const targets=r.segments.filter(visible).sort((a,b)=>Math.hypot(a.x-t.x,a.y-t.y)-Math.hypot(b.x-t.x,b.y-t.y));for(let i=0;i<w.count*(w.legendary?2:1);i++)homing(r,w,isActionRun(r)?410:r.heroX+(i%3-1)*14,isActionRun(r)?555:r.heroY-25,targets[i%targets.length]);}
 ...
 if(base.type==='dragon'){const head=(isActionRun(r)?r.segments.find(s=>visible(s)&&s.snakeId===t.snakeId):null)||r.segments.find(visible)||t;for(let i=0;i<(w.legendary?2:1);i++)homing(r,w,isActionRun(r)?35:r.heroX+(i?25:-10),isActionRun(r)?320:r.heroY-25,head,'dragon');}
}
const chargeRate=r=>r.mode==='tournament'?1:SPECIAL_CHARGE*SPECIAL_RANKS[(r.specialRank||1)-1].charge;
export function ultimate(r){if(r.state!=='playing'||r.charge<100)return false;
 // The tournament keeps its original free Market Crash. Campaign specials are bought in the
 // Foundry; without one equipped there is nothing to fire.
 if(r.mode==='tournament'){r.slowUntil=r.time+3;r.slowAmount=.7;const w=r.weapons[0];for(const s of r.segments)if(visible(s))hit(r,s,w,w.damage*4);r.charge=0;fx(r,{type:'ultimate',x:240,y:340,r:400,color:'#ffd27b',life:1.1});r.events.push({type:'ultimate',special:'crash'});return true;}
 const id=r.special;if(!id||!SPECIALS[id]||!r.weapons.length)return false;
 const best=r.weapons.reduce((b,w)=>w.damage>b.damage?w:b),amount=best.damage*SPECIALS[id].multiplier*SPECIAL_RANKS[(r.specialRank||1)-1].damage,targets=r.segments.filter(visible);
 if(id==='crash'){r.slowUntil=r.time+3;r.slowAmount=.7;for(const s of targets)hit(r,s,best,amount);fx(r,{type:'ultimate',x:240,y:340,r:620,color:'#ffd27b',life:1.4});}
 if(id==='wave'){fx(r,{type:'sweep',y:-40,speed:900,amount,weapon:best.id,hits:[],color:'#9ceaff',life:1.1});r.headDistance=Math.max(CAMPAIGN_ENTRY,r.headDistance-200);if(isActionRun(r))for(const snake of r.snakes)snake.distance-=200;for(const s of r.segments)s.retreat=0;updatePositions(r);}
 if(id==='halving'){for(const s of targets)hit(r,s,best,Math.min(s.hp*.15,amount));fx(r,{type:'ultimate',x:240,y:340,r:520,color:'#a5c9ff',life:1.2});}
 if(id==='forkbomb'){for(const s of targets){const others=targets.filter(x=>x.id!==s.id).sort((a,b)=>Math.hypot(a.x-s.x,a.y-s.y)-Math.hypot(b.x-s.x,b.y-s.y)).slice(0,2);for(const t of others){homing(r,best,s.x,s.y,t,'fragment',0,amount/best.damage);r.bullets.at(-1).ignoreId=s.id;}}fx(r,{type:'ultimate',x:240,y:340,r:460,color:'#ffa778',life:1});}
...
export function tick(r,dt){
 ...
 r.frenzy=!isActionRun(r)&&r.mode!=='tournament'&&(r.entered||0)<FRENZY_SECTIONS?1+(FRENZY_SPEED/d.speed-1)*(1-frenzyReduction(r.boons)):1;
 advanceSnake(r,dt,(isActionRun(r)?actionSpeed(r):(r.mode==='tournament'?Math.min(140,11+r.wave*2):waveMovement(r.wave))*c.speed*d.speed)*(r.boons.includes('slow')?.85:1)*encounterPhase(r).speed*(r.rootUntil>r.time?0:1)*r.frenzy);
 for(const s of r.segments){s.flash=Math.max(0,s.flash-dt);if(s.slipStacks>0&&s.slipUntil<=r.time){s.slipStacks--;s.slipUntil=r.time+1;}if(s.burn>0){s.burn=Math.max(0,s.burn-dt);const amount=s.burnDps*dt*(s.markedUntil>r.time?1.25:1)*(1+.08*(s.slipStacks||0));s.hp-=amount;r.charge=Math.min(100,r.charge+amount*.012*chargeRate(r)/(r.mode==='tournament'?1:encounterHealth(r.chapter+1)));const w=r.weapons.find(w=>w.id===s.burnWeapon);if(w)w.totalDamage+=amount;}if(s.hp>0&&(s.regen||!s.head&&encounterPhase(r).id==='mend'))s.hp=Math.min(s.maxHp,s.hp+s.maxHp*(encounterPhase(r).id==='mend'?.009:.006)*dt);}
 const canFire=advanceHeat(r,dt),t=target(r);if(t){if(!r.manual&&!isActionRun(r)){const p=aimPoint(t,r.heroX,r.heroY);r.aimX=p.x;r.aimY=p.y;}for(const w of r.weapons){w.timer=Math.max(0,w.timer-dt);if(w.debtUntil>r.time){const debtor=r.segments.find(s=>s.id===w.debtTarget&&s.hp>0);if(!debtor){w.debtUntil=0;w.debtTarget=0;w.timer=w.legendary?0:Math.max(0,w.timer-2.5);}else continue;}if(w.timer<=0&&canFire){w.timer=w.cooldown;fire(r,w,t);}}}
 ...
 for(const e of [...r.effects]){e.life-=dt;
  ...
  if(e.type==='airstrike'&&e.life<=.12&&!e.trigger){e.trigger=true;if(source){area(r,source,e.x,e.y,e.r,source.damage);if(e.napalm){hazard(r,source,e.x,e.y,e.r,source.burnTime,source.damage*.35,'fire');for(const s of r.segments)if(visible(s)&&sectionDistance(s,e.x,e.y)<e.r)ignite(s,source);}fx(r,{type:'blast',x:e.x,y:e.y,r:e.r,color:e.color,life:.45});}}
  if(e.type==='trap'&&e.armed&&r.segments.some(s=>visible(s)&&sectionDistance(s,e.x,e.y)<22)){e.armed=false;e.life=0;if(source)area(r,source,e.x,e.y,e.r);r.rootTargets=r.segments.filter(s=>visible(s)&&sectionDistance(s,e.x,e.y)<e.r+17).map(s=>s.id);r.rootUntil=r.rootTargets.length?r.time+2:0;if(source?.legendary&&!e.secondary){const nearest=r.segments.filter(visible).sort((a,b)=>sectionDistance(a,e.x,e.y)-sectionDistance(b,e.x,e.y))[0];if(nearest){const distance=nearest.distance,p=isActionRun(r)?routePoint(r,distance,nearest.snakeId):pathPoint(distance,r.chapter);fx(r,{type:'trap',id:++r.castId,weapon:source.id,distance,x:p.x,y:p.y,r:source.radius,color:e.color,life:8,armed:true,secondary:true});}}fx(r,{type:'blast',x:e.x,y:e.y,r:e.r,color:e.color,life:.5});}
  if(e.type==='lambo'&&source){e.travel+=420*dt;e.distance=e.start+e.direction*Math.min(e.travel,e.range);const p=isActionRun(r)?routePoint(r,e.distance,e.snakeId||0):pathPoint(e.distance,r.chapter);Object.assign(e,p);
...
if(e.type==='hazard'){e.tick-=dt;if(e.tick<=0){e.tick=.4;const source=r.weapons.find(w=>w.id===e.weapon);if(source)area(r,source,e.x,e.y,e.r,e.amount);}}if(e.type==='sweep'){e.y+=e.speed*dt;const w=r.weapons.find(w=>w.id===e.weapon);for(const s of r.segments)if(visible(s)&&!e.hits.includes(s.id)&&sectionPoints(s).some(p=>p.y<=e.y)){e.hits.push(s.id);if(w)hit(r,s,w,e.amount);}}
  if(e.type==='meteor'&&e.life<=.15&&!e.trigger){e.trigger=true;const w=r.weapons.find(w=>w.id===e.weapon);if(w){area(r,w,e.x,e.y,e.r);if(w.legendary&&!e.echo)fx(r,{type:'meteor',x:e.x,y:e.y,r:e.r,color:e.color,life:.6,weapon:w.id,trigger:false,echo:true});}r.events.push({type:'impact'});}}
