...
Talos Prototype resolves each accepted HP threshold in two deterministic stages. The threshold clamp, phase exit/entry, and warning happen immediately. When that threshold's warning delay elapses, its exposed-core coefficient, resistance overrides, and statuses begin together and its Scout-pod release schedule is accepted. Every Scout `firstDelayTicks` and `intervalTicks` value is relative to that release point, not to the original threshold hit. Once accepted, those scheduled Scout jobs retain the captured route/lineage data and still run if Talos later dies or leaks; terminal parent state cannot cancel them.
