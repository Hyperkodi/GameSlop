...
try{
  for(const spec of Object.values(SOUND_BANK)){
    const response=await fetch(base+spec.file);assert.equal(response.status,200);assert.match(response.headers.get('content-type'),/audio\/wav/);
  ...
  assert.deepEqual(report.decoded,{count:20,failed:[]});
  ...
  report.mix=await page.evaluate(async()=>{
    ...
    ['power','dash','ghost','combo','warning','gate'].forEach((id,i)=>play(id,.3+i*.05));
  ...
  assert.equal(report.playback.starts,20);assert.ok(report.playback.voices<=8);assert.equal(report.playback.stopped,0);assert.ok(report.playback.silent&&report.playback.restarted);
  ...
  assert.equal(await phone.locator('#sounds button').count(),20);assert.ok(await phone.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
