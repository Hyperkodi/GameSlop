...
try{
  ...
  await phone.waitForFunction(()=>document.getElementById('cabinet').classList.contains('auto-landscape'),null,{timeout:3000}).catch(async error=>{console.log('ROTATION',await phone.evaluate(()=>({width:innerWidth,height:innerHeight,touch:navigator.maxTouchPoints,phase:window.__pacQA.phase,classes:document.getElementById('cabinet').className,fullscreen:!!document.fullscreenElement})));throw error;});
  await phone.locator('[data-dir="3"]').tap();await phone.waitForFunction(()=>!!document.fullscreenElement||document.getElementById('cabinet').classList.contains('full-window'));
