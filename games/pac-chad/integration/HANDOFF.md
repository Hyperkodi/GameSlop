...
- `Recorder` packs two four-bit inputs per byte. Recording supports 864,000 ticks (four hours), up to 576,000 base64 characters. This no longer fits the current 16 KiB platform limit; use a reviewed bounded upload/storage path before ranked integration. Longer local sessions remain playable but have no replay export. Player identity and run authorization do not come from this envelope.
...
2. Add an immutable scoring ruleset: `pac-chad-campaign-v3`, metric points, higher-is-better, ten-level campaign with no gameplay time limit, normal difficulty, three lives. Pin the whole deployed build and its relevant assets. Use a new ruleset ID when logic changes. The machine-readable candidate is `ruleset.json`; keep it `ranked: false` until reviewed and wired.
