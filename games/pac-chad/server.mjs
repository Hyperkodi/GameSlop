...
const types={'.html':'text/html; charset=utf-8','.css':'text/css','.mjs':'text/javascript','.js':'text/javascript','.json':'application/json','.png':'image/png','.webp':'image/webp','.mp3':'audio/mpeg','.wav':'audio/wav'};
export function createServer(){return http.createServer(async(req,res)=>{
  try{
    ...
    if(relative.split(/[\\/]/).some(v=>v==='..'||v.startsWith('.'))||!/^(index\.html|sound-lab\.html|style\.css|js\/[a-z-]+\.mjs|assets\/[a-z-]+\.(png|webp|mp3|wav)|integration\/gameslop-sdk\.js)$/.test(relative)){res.writeHead(404);res.end('Not found');return;}
