...
- Superseded checkpoint before the completion audit: the six-to-twelve-minute session length remained a pacing target rather than a forced timer. The deterministic bot chooses cards instantly and excludes time spent reading choices. Early runs can be shorter and unlucky low-investment builds longer. A trial that delayed campaign choices introduced a loss and was removed. The final report explicitly includes the observed timing distribution rather than padding runs with waiting or hiding outliers. Health, speed and wave-count formulas remain unchanged.

## Completion audit

The earlier pacing limitation is resolved. Campaign entrance and continuous body geometry now derive from an eight-minute travel budget, preserving the required health, speed and wave curves. The harness now fails any canonical clear outside six to twelve minutes. It funds the six-weapon core and uses investment-aware choices, keeping unfunded control unlocks eligible but deprioritizing them for speed. Test accounts no longer pre-clear the current encounter. Roots release when the struck sections die.

The audit also adds Slippage stack pips, validates saved spacing and root IDs, derives encounter descriptions from actual traits and phases, and updates offline cache v2.0.1. Final results and limitations are in VERIFICATION.md; REQUIREMENTS-AUDIT.md maps every brief section.
