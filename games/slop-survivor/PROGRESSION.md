...
The snake enters at 5x speed until three sections are on the board each wave, then settles to normal speed, so no time is spent watching an empty arena. On Hard and Impossible the starting-bonus screen sometimes offers a frenzy boon (Speed Bump, Circuit Halt or Trading Halt) that slows or removes that rush. The tournament has no frenzy.

Campaign snakes enter at the portal. Body-piece spacing is derived from a travel budget of seven minutes at level 1 rising to eight minutes by level 31, with the original 32-pixel minimum. Levels carry 33 to 64 base pieces. Battle chests arrive every `chestStride` kills, derived from the level's total pieces so every level yields roughly 30 cards; the tournament keeps its fixed four-kill stride. The rescue chest that fires after a card drought waits 24 seconds on Easy and 28 on Hard and Impossible. Hard is 2.2x health at 1.16x speed with a double-health head; Impossible is 2.8x health at 1.42x speed with a quadruple-health head. A heavy head can reach the vault, breach and be pushed back before it dies, which is what makes the harder tiers cost shields. Hard pays 2.8x and grants cores every nine levels, because Impossible is now expected to be lost about a third of the time even by a player on the curve. Health, movement multipliers, piece caps and wave counts retain the specified formulas. The shared curve model prints the derivation. There is no waiting state or minimum victory timer. Tournament geometry stays unchanged. Bear Trap roots living struck sections for two seconds and releases when those victims die.

The full balance harness requires every Easy win to fall between 360 and 720 active seconds, allows Hard 840 and Impossible 960 because breaches push the snake back, and requires the harder tiers to overrun their feed budget more than Easy does. The final sweep records 389 to 910 seconds, median 571, using an investment-aware speed-focused card policy. Hard cost shields in 10% of levels and Impossible in 38%; Impossible won 86 of 100 first attempts and every level within three. Card-reading time is excluded; control-heavy builds and other player choices can take longer. See VERIFICATION.md for assumptions and complete results.
