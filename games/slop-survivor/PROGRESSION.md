...
Campaign snakes enter at the portal. Body-piece spacing is derived from a travel budget of seven minutes at level 1 rising to eight minutes by level 31, with the original 32-pixel minimum. Levels carry 33 to 64 base pieces. Battle chests arrive every `chestStride` kills, derived from the level's total pieces so every level yields roughly 30 cards; the tournament keeps its fixed four-kill stride. The rescue chest that fires after a card drought waits 24 seconds on Easy and 28 on Hard and Impossible. Health, movement multipliers, piece caps and wave counts retain the specified formulas. The shared curve model prints the derivation. There is no waiting state or minimum victory timer. Tournament geometry stays unchanged. Bear Trap roots living struck sections for two seconds and releases when those victims die.

The full balance harness requires every Easy win to fall between 360 and 720 active seconds, allows Hard 840 and Impossible 960 because breaches push the snake back, and requires the harder tiers to overrun their feed budget more than Easy does. The final sweep records 389 to 939 seconds, median 576, using an investment-aware speed-focused card policy; Impossible level 74 needed one alternate seed. Card-reading time is excluded; control-heavy builds and other player choices can take longer. See VERIFICATION.md for assumptions and complete results.
