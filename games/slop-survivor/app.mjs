...
function finish(){releaseInputs();const reward=completeRun(save,run);if(!reward)return;persist();sound.event(reward.win?'won':'lost');if(reward.tournament){finishTournament(reward);return;}const d=run.difficulty,c=run.chapter;let message=reward.win?(reward.first?d==='easy'?`Hard unlocked for this chapter.${c<CHAPTERS.length-1?' Chapter '+(c+2)+' is now open.':''}`:d==='hard'?'Hell unlocked for this chapter.':'Hell cleared. That vault is in good hands.':'Another clean defense. Your rewards are in the vault.'):'Your permanent upgrades are safe. Focus critical hits on armor, and try a different weapon mix.';
 if(reward.win&&d==='easy'){const w=WEAPONS.find(w=>w.unlock===c+1);if(w)message+=` New weapon: ${w.name}. Equip it in the Armory.`;}
