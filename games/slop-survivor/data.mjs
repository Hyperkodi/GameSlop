...
for(const w of WEAPONS){[w.grade,w.discovery,w.class]=roster[w.id];}
...
function migrateV1(raw){
 const s=JSON.parse(JSON.stringify(raw));s.version=VERSION;s.parts={...s.shards};delete s.shards;s.ranks={};s.owned=WEAPONS.filter(w=>w.unlock===0||raw.clears?.[`${w.unlock-1}:normal`]).map(w=>w.id);s.cores=0;s.blueprints=0;s.clears={};s.best={};
