...
function ready(){const save=defaultSave(100),r=createRun(save,0,'normal',77);chooseBoon(r,'damage');chooseUpgrade(r,r.choices[0].id);r.weapons=[];r.pending=0;r.lastChoice=10000;return {save,r};}
...
test('a projectile can hit either end of a curved section and piercing never damages its four pieces repeatedly',()=>{
 for(const endpoint of [0,12]){const {r}=ready();const w=makeWeapon('coin');w.timer=10;w.crit=0;r.weapons=[w];const section=r.segments[2],p=section.points[endpoint],hp=section.hp;
...
test('partial tail sections preserve chapter length and armor retains its one-in-four frequency',()=>{
 const {r}=ready();r.chapter=1;r.wave=0;spawnWave(r);
 assert.equal(r.segments.reduce((v,s)=>v+s.pieces,0),28);assert.equal(r.segments.at(-1).pieces,3);
