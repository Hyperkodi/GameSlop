...
// Easy keeps every shield and wins every level: it is the progression path. Hard must win
// every level on the canonical seed but its double-weight head must cost shields in at
// least one level in twenty. Impossible is meant to be damn near impossible for a person:
// the optimal bot must win between 60% and 95% of levels on the canonical seed (the upper
// bound stops it drifting trivial), lose shields in at least 30%, and every level (level
...
const economy=economyReplay();assert.ok(economy.farms<=40,'economy requires excessive repeat farming');for(const row of economy.rows)console.log(JSON.stringify({check:'economy',...row}));
